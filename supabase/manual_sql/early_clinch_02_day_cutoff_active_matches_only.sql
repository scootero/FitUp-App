-- =============================================================================
-- early_clinch_02_day_cutoff_active_matches_only.sql  (MANUAL APPLY)
-- =============================================================================
-- Purpose:
--   Prevent day_cutoff_check from processing days whose parent match is no
--   longer active.
--
-- Notes:
...
DECLARE
  v_match_day_id uuid;
BEGIN
  -- Phase 1: pending participants past local cutoff -> confirmed
  -- Only process days whose parent match is active.
  ...
  -- Phase 2: all participants already confirmed, day not finalized, cutoff passed
  -- for every participant. Only process days whose parent match is active.
  FOR v_match_day_id IN
    SELECT md.id
    FROM match_days md
    JOIN matches m
