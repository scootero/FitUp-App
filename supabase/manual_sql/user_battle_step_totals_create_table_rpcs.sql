-- Battle Steps (Stats top card): table + RPCs + backfill
-- Run once in Supabase Dashboard → SQL Editor (idempotent; safe to re-run).
--
-- Prerequisite: `user_daily_step_totals` optional but recommended
...
grant execute on function public.reconcile_user_battle_step_total(uuid, date, integer, text) to authenticated, service_role;
...
-- ---------------------------------------------------------------------------
-- Backfill (historical finalized battle days)
-- ---------------------------------------------------------------------------
