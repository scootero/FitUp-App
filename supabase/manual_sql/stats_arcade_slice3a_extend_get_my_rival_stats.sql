-- Slice 3A (manual SQL): extend get_my_rival_stats() with rival detail fields used by Stats Arcade.
-- Run manually in Supabase SQL Editor. Do not run from the app/agent.
--
-- If RETURNS TABLE columns change, DROP first (42P13):
--   DROP FUNCTION IF EXISTS public.get_my_rival_stats(int);
--
-- Additive changes only:
-- - existing columns keep prior semantics
-- - new columns are nullable and optional for backward-compatible clients
