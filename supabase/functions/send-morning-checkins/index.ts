...
serve(async (request) => {
  ...
  try {
    const { data: activeRows, error: activeError } = await supabaseAdmin
      .from("matches")
      .select("id, metric_type, duration_days, scoring_mode")
      .eq("state", "active");
    if (activeError) {
      throw activeError;
    }
    ...
    for (const match of matches) {
      ...
      for (const row of participants) {
        ...
        if (raw == null) baselines.set(uid, null);
        else baselines.set(uid, toNumber(raw));
      }
      ...
      const isBalancedSteps = scoringMode === "balanced" && match.metric_type === "steps";
      ...
      for (const participant of participants) {
        ...
        const myTotal = totals.get(String(participant.user_id)) ?? 0;
        const theirTotal = opponentId ? totals.get(String(opponentId)) ?? 0 : 0;

        let standingLabel: string;
        if (isBalancedSteps && opponentId) {
          const myB = baselines.get(String(participant.user_id)) ?? null;
          const ob = baselines.get(String(opponentId)) ?? null;
          const myBS = battleScore(myTotal, myB, ob);
          const theirBS = battleScore(theirTotal, ob, myB);
          standingLabel = myBS === theirBS ? "tied" : myBS > theirBS ? "ahead" : "behind";
        } else {
          standingLabel = myTotal == theirTotal ? "tied" : myTotal > theirTotal ? "ahead" : "behind";
        }
...
async function alreadySentToday(userId: unknown, eventType: string, matchId: unknown) {
...
async function fetchDisplayName(userId: unknown) {
...
async function currentDayNumber(matchId: unknown) {
...
async function currentDayTotals(matchId: unknown) {
...
async function loadSeriesScores(matchId: unknown) {
