...
serve(async (request) => {
  ...
  try {
    const body = await readJsonBody<OnAllAcceptedRequest>(request);
    const matchId = body.match_id?.trim();
    if (!matchId) {
      return jsonResponse(400, { error: "match_id is required." });
    }

    const { data: activated, error: rpcError } = await supabaseAdmin.rpc(
      "activate_match_with_days",
      { p_match_id: matchId },
    );
    if (rpcError) {
      throw rpcError;
    ...
    if (activated !== true) {
      return jsonResponse(200, { status: "skipped", match_id: matchId });
    }
