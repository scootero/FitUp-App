...
final class SessionStore: ObservableObject {
    ...
    func signInWithEmail(email: String, password: String) async {
        ...
        do {
            ...
            authErrorMessage = Self.userFacingEmailAuthError(error, email: email, isSignUp: false)
            AppLogger.log(
                category: "auth",
                level: .error,
                ...
                metadata: [
                    ...
                    "email_length": String(email.count),
    ...
    func signUp(email: String, password: String, displayName: String) async {
        ...
        do {
            ...
            authErrorMessage = Self.userFacingEmailAuthError(error, email: email, isSignUp: true)
            isAuthenticated = false
            currentProfile = nil
            postAuthNameFieldPrefill = nil
            postAuthDisplayNameStepComplete = true
    ...
    func signOut() async {
        ...
        do {
            ...
            if let signedOutUserId {
                // Insert while the JWT is still valid. After sign-out these rows fail RLS (42501).
                await ProductAnalytics.endForegroundSessionForAuthChange(profileId: signedOutUserId)
                await ProductAnalytics.trackAwaitingInsert(
                    ProductAnalytics.Event.authSignOut,
    ...
    private func applyPostAuthDisplayNameStateAfterRestore(profile: Profile) {
        ...
        if UserDefaults.standard.object(forKey: key) == nil {
            // First launch with this key: treat auto-generated "FitOff …" / legacy "FitUp …" as incomplete; real names as complete (app upgrade / existing users).
    ...
    /// GoTrue uses the same "Invalid login credentials" text for a wrong password and an unknown email.
    private static func userFacingEmailAuthError(_ error: Error, email: String, isSignUp: Bool) -> String {
        let raw = error.localizedDescription
        let lowered = raw.lowercased()
        if lowered.contains("invalid login credentials") {
            return "That email and password don’t match an account. Sign in with the same email you created (\(email))."
        }
        if lowered.contains("invalid format") {
            return "Enter a full email address, like name@email.com."
        }
        if lowered.contains("already registered") || lowered.contains("user already exists") {
            return "That email already has an account. Switch to Sign In and use the password from when it was created."
        }
        if isSignUp, lowered.contains("password") {
            return raw
        }
        return raw
    }
