...
serve(async (request) => {
  ...
  try {
    const body = await readJsonBody<CompleteMatchRequest>(request);
...
async function loadParticipantIds(matchId: string): Promise<string[]> {
...
function resolveSeriesWinner(scores: Map<string, number>): string | null {
