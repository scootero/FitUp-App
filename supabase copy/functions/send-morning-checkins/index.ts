...
serve(async (request) => {
  ...
  try {
    ...
    const matches = (activeRows ?? []) as ActiveMatch[];
    let dispatched = 0;
...
async function currentDayNumber(matchId: string): Promise<number> {
  const { data, error } = await supabaseAdmin
    .from("match_days")
...
async function currentDayTotals(matchId: string): Promise<Map<string, number>> {
...
async function loadSeriesScores(matchId: string): Promise<Map<string, number>> {
